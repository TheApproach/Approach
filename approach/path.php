...
enum path: int
{
	...
	public function get(string $project_dir = null): string
	{
		...
		// Enforce trailing / on project_dir, support_dir and static_dir
		if( $project_dir !== null ) $project_dir = rtrim($project_dir, '/') . '/';
		if( $support_dir !== null ) $support_dir = rtrim($support_dir, '/') . '/';
		if( $static_dir !== null ) $static_dir = rtrim($static_dir, '/') . '/';
