...
trait discoverability
{
	...
	*/
    ...
    public static function define_profile($caller, $info): void
    {
        ...
        $php .= 'public static function multi_match($which, $what)' . PHP_EOL . '{' . PHP_EOL;
        $php .= "\t" . '$result = null;' . PHP_EOL . "\t" . 'switch($which) {' . PHP_EOL;

        foreach (static::$aspect_metadata as $aspect => $matches) {
            $uc_aspect = ucfirst($aspect);
            if (empty($info[$aspect])) continue; // if there was no info for this aspect, skip its functions
            $php .= "\t\t" . 'case Aspect::' . $aspect . ':' . PHP_EOL;
            $php .= "\t\t\t" . '$result = Self' . $uc_aspect . '::match($what);' . PHP_EOL;
            $php .= "\t\t\t" . 'break;' . PHP_EOL;
        }
        ...
        $php .= "\t" . '}' . PHP_EOL . "\t" . 'return $result;' . PHP_EOL . '}' . PHP_EOL;
