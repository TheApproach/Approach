...
class Server extends Resource
{
    ...
    )
    {
		...
		foreach (get_class_vars(self::class) as $key => $value){
			if (!isset($this->{$key}) && defined(self::class . '::' . $key))
			{
				$this->{$key} = constant(self::class . '::' . $key);
			}
		}
	...
	public function MintResourceClass(
		...
		if(empty($constants)){
			if(!empty($this->host)) 
				$constants[]= 'public const HOST = \''.$this->host.'\';';
			if(!empty($this->user)) 
				$constants[]= 'public const USER = \''.$this->user.'\';';
			// if(!empty($this->pass)) 
			// 	$constants[]= 'public const PASS = \''.$this->pass.'\';';
			if(!empty($this->database)) 
				$constants[]= 'public const DATABASE = \''.$this->database.'\';';
			if(!empty($this->port)) 
				$constants[]= 'public const PORT = \''.$this->port.'\';';
			if(!empty($this->socket)) 
				$constants[]= 'public const SOCKET = \''.$this->socket.'\';';
			// if(!empty($this->ssl_key)) 
			// 	$constants[]= 'public const SSL_KEY = \''.$this->ssl_key.'\';';
			if(!empty($this->ssl_cert)) 
				$constants[]= 'public const SSL_CERT = \''.$this->ssl_cert.'\';';
			if(!empty($this->ssl_ca)) 
				$constants[]= 'public const SSL_CA = \''.$this->ssl_ca.'\';';
			if(!empty($this->ssl_capath)) 
				$constants[]= 'public const SSL_CAPATH = \''.$this->ssl_capath.'\';';
			if(!empty($this->ssl_cipher)) 
				$constants[]= 'public const SSL_CIPHER = \''.$this->ssl_cipher.'\';';
			if(!empty($this->charset)) 
				$constants[]= 'public const CHARSET = \''.$this->charset.'\';';
			if(!empty($this->collation)) 
				$constants[]= 'public const COLLATION = \''.$this->collation.'\';';
			if(!empty($this->timeout)) 
				$constants[]= 'public const TIMEOUT = \''.$this->timeout.'\';';
			if(!empty($this->persistent)) 
				$constants[]= 'public const PERSISTENT = \''.$this->persistent.'\';';
			if(!empty($this->skip_connection)) 
				$constants[]= 'public const SKIP_CONNECTION = \''.$this->skip_connection.'\';';
			if(!empty($this->is_galera)) 
				$constants[]= 'public const IS_GALERA = \''.$this->is_galera.'\';';
		}
