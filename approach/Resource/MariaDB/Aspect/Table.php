...
class Table extends discover
{
    ...
    public static function map_data_types($type)
    {
        ...
        switch (true) {
            ...
            case (is_int(strpos($type, 'bigint'))):
                $r = 'int';
                break;

            case (is_int(strpos($type, 'decimal'))):
            case (is_int(strpos($type, 'float'))):
            case (is_int(strpos($type, 'double'))):
                $r = 'float';
                break;
            ...
            case (is_int(strpos($type, 'text'))):
                $r = 'string';
                break;    # accounts for tinytext, mediumtext, longtext..

            case (is_int(strpos($type, 'blob'))):
                $r =  'blob';
                break;    # handed off to Service\Decoder::decode, accounts for all size of blob
            case (is_int(strpos($type, 'datetime'))):
                $r =  'datetime';
                break;
            case (is_int(strpos($type, 'timestamp'))):
                $r =  'timestamp';
                break;
            case (is_int(strpos($type, 'date'))):
                $r =  'date';
                break;    # must occur after datetime
            case (is_int(strpos($type, 'time'))):
                $r =  'time';
                break;    # must occur after datetime & timestamp
            case (is_int(strpos($type, 'enum'))):
                $r =  'enum';
                break;
            case (is_int(strpos($type, 'set'))):
                $r =  'set';
                break;
            case (is_int(strpos($type, 'json'))):
                $r =  'json';
                break;
            case (is_int(strpos($type, 'xml'))):
                $r =  'xml';
                break;
            default:
                $r =  'string';
    ...
    public static function equipState($columns, $caller)
    {
        ...
        foreach ($columns as $key => $value) {
            switch ($value) {
                case 'DATABASE_CLASS':
                    $data[$index] = $resource_ns . '\\' . $server_label . '\\' . $safe_database_name;
                    break;
                case 'SERVER_CLASS':
                    $data[$index] = $resource_ns . '\\' . $server_label;
                    break;
                case 'CONNECTOR_CLASS':
                    $data[$index] =  '\Approach\Service\MariaDB\Connector';
                    break;
                case 'RESOURCE_CLASS':
                    $data[$index] = $resource_ns;
                    break;
                case 'DATABASE_NAME':
                    $data[$index] = $safe_database_name;
                    break;
                case 'SERVER_NAME':
                    $data[$index] = $caller->server->label ?? 'NULL';
                    break;
                case 'RESOURCE_PROTO':
                    $data[$index] = 'MariaDB';
                case 'NAME':
                    $data[$index] = $caller->table['TABLE_NAME'] ?? 'NULL';
                    break;
                case 'COMMENT':
                    $data[$index] = $caller->table['TABLE_COMMENT'] ?? 'NULL';
                    break;
                case 'ENGINE':
                    $data[$index] = $caller->table['ENGINE'] ?? 'NULL';
                    break;
                case 'ROW_FORMAT':
                    $data[$index] = $caller->table['ROW_FORMAT'] ?? 'NULL';
                    break;
                case 'TABLE_COLLATION':
                    $data[$index] = $caller->table['TABLE_COLLATION'] ?? 'NULL';
                    break;
                case 'CREATE_OPTIONS':
                    $data[$index] = $caller->table['CREATE_OPTIONS'] ?? 'NULL';
                    break;
                case 'TABLE_ROWS':
                    $data[$index] = $caller->table['TABLE_ROWS'] ?? 'NULL';
                    break;
                case 'AVG_ROW_LENGTH':
                    $data[$index] = $caller->table['AVG_ROW_LENGTH'] ?? 'NULL';
                    break;
                case 'DATA_LENGTH':
                    $data[$index] = $caller->table['DATA_LENGTH'] ?? 'NULL';
                    break;
                case 'MAX_DATA_LENGTH':
                    $data[$index] = $caller->table['MAX_DATA_LENGTH'] ?? 'NULL';
                    break;
                case 'INDEX_LENGTH':
                    $data[$index] = $caller->table['INDEX_LENGTH'] ?? 'NULL';
                    break;
                case 'DATA_FREE':
                    $data[$index] = $caller->table['DATA_FREE'] ?? 'NULL';
                    break;
                case 'AUTO_INCREMENT':
                    $data[$index] = $caller->table['AUTO_INCREMENT'] ?? 'NULL';
                    break;
                case 'CREATE_TIME':
                    $data[$index] = $caller->table['CREATE_TIME'] ?? 'NULL';
                    break;
                case 'UPDATE_TIME':
                    $data[$index] = $caller->table['UPDATE_TIME'] ?? 'NULL';
                    break;
                case 'CHECK_TIME':
                    $data[$index] = $caller->table['CHECK_TIME'] ?? 'NULL';
                    break;
                case 'CHECKSUM':
                    $data[$index] = $caller->table['CHECKSUM'] ?? 'NULL';
                    break;
                case 'TABLE_COMMENT':
                    $data[$index] = $caller->table['TABLE_COMMENT'] ?? 'NULL';
                    break;
