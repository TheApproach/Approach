...
class ImprintCest
{
    ...
    */
    ...
    public function checkNodeName(UnitTester $I)
    {
        ...
        foreach($use_cases as $key => $use_case){
            ...
            $node_name = $imprint->exportNodeName(...$use_case);
            $I->assertEquals(
                $samples[$key], 
                $node_name, 
                ' $node->generateNodeName() should return ' . $samples[$key] . ' but returned ' . $node_name
            );
        }
    }
    ...
    public function checkExportDependencies(UnitTester $I){
        $imprint = new Imprint(
            imprint: 'test/hellotoken.xml',
            imprint_base: $this->scope->getPath(path::pattern)
        );
        ...
        foreach($use_cases as $key => $use_case){
            //... simulate instrucitons in exportTree() timeline leading up to where node name is instantiated
            $dependencies = $imprint->exportDependencies(...$use_case);
            $I->assertEquals(
                $samples[$key], 
                $dependencies, 
                ' $node->generateDependencies() should return ' . $samples[$key] . ' but returned ' . $dependencies
            );
        }
    }
	...
	*/
    ...
    public function exportTreeBuilds(UnitTester $I)
    {
        ...
        $imprint->Mint('hellotoken/hello'); // generate all  files
